...
public class LightBulb {
	...
	public void setOn(boolean on) {
		try {
			...
			LoggerFactory.getLogger(TradfriGateway.class).warn(ex.toString());
		}
		this.on = on;
	}
	...
	protected void parseResponse(CoapResponse response) {
		...
		gateway.getLogger().info(response.getResponseText());
		try {
			...
			LoggerFactory.getLogger(TradfriGateway.class).error("Cannot update bulb info: error parsing the response from the gateway. Response is: {}", response.getResponseText());
			e.printStackTrace();
		}
		if (updateListeners) {
