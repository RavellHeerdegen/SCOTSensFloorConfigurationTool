...
/**
 * @author https://github.com/ffleurey/ThingML-Tradfri and some minor changes by Florian Jungermann
 *
 */
public class TradfriGateway implements Runnable {
    ...
    protected String security_identity;
    ...
    public TradfriGateway(String gateway_ip, String security_identity, String security_key) {
    ...
    public void setSecurityIdentity(String security_identity) {
        this.security_identity = security_identity;
    }
    ...
    private Logger logger = LoggerFactory.getLogger(TradfriGateway.class);
    ...
    public void run() {
        ...
        if (dicoverBulbs()) {
        	...
        	logger.info("Discovered " + bulbs.size() + " Bulbs.");
            for (TradfriGatewayListener l : listners)
                l.gateway_started();
            try {
                while (running) {
                    ...
                    logger.info("Polling bulbs status...");
                    for (TradfriGatewayListener l : listners)
                        l.polling_started();
            	...
            	logger.error("Error while polling bulbs", ex);
            }
        }
        running = false;
        for (TradfriGatewayListener l : listners)
    ...
    protected boolean dicoverBulbs() {
        ...
        try {
        	...
        	logger.error("Error parsing response from the Tradfri gateway", e);
            return false;
    ...
    protected void initCoap() {
        ...
        builder.setPskStore(new StaticPskStore(security_identity, security_key.getBytes()));
    ...
    protected CoapResponse get(String path) {
    	...
    	logger.info("GET: " + "coaps://" + gateway_ip + "/" + path);
        CoapClient client = new CoapClient("coaps://" + gateway_ip + "/" + path);
        client.setEndpoint(coap);
        CoapResponse response = client.get(1);
        if (response == null) {
            logger.warn("Connection to Gateway timed out, please check ip address or increase the ACK_TIMEOUT in the Californium.properties file");
        }
        return response;
    }
